...
class FileSystem:
    ...
    def createCheckpoint(self,diskId,checkPtId):
        if not diskId in self.diskList:
            print "There is no such diskId present"
            return False
        if checkPtId in self.diskList[diskId].snapshotList:
            print "This checkpoint Id already exists"
            return False
        currSnapshot=[]
        for blockNum in self.diskList[diskId].blockList:
            newBlockData=bytearray(self.blockSize)
            self.readBlock(blockNum+1,newBlockData)
            currSnapshot.append((newBlockData,copy.deepcopy(self.blocksMetaData[blockNum])))
        self.diskList[diskId].snapshotList[checkPtId]=(currSnapshot)
        return True

    def rollBack(self,diskId,checkPtId):
        if not diskId in self.diskList:
            print "There is no such diskId present"
            return False
        if not checkPtId in self.diskList[diskId].snapshotList:
            print "This checkpoint Id is INVALID!"
            return False
        oldSnap=self.diskList[diskId].snapshotList[checkPtId]
        tempCount=0
        for blockNum in self.diskList[diskId].blockList:
            (oldBlockData,oldBlockMetaData)=oldSnap[tempCount]
            tempCount+=1
            self.writeBlock(blockNum+1,oldBlockData)
            self.blocksMetaData[blockNum]=oldBlockMetaData
        return True



def runSnapshotTests():
    writeData1 = bytearray(b'Aravali')
    writeData2 = bytearray(b'Zanskar')
    writeData3 = bytearray(b'Himadri')
    writeData4 = bytearray(b'Karakoram')
    writeData5 = bytearray(b'Kumaon')
    writeData6 = bytearray(b'Nilgiri')
    writeData7 = bytearray(b'Jwala')
    writeData8 = bytearray(b'Girnar')
    myFileSystem=FileSystem(100)
    myFileSystem.createDisk("ayushDisk",20)
    myFileSystem.writeDisk("ayushDisk", 3, writeData1)
    myFileSystem.writeDisk("ayushDisk", 7, writeData2)
    myFileSystem.writeDisk("ayushDisk", 8, writeData3)
    myFileSystem.writeDisk("ayushDisk", 9, writeData4)

    readBuffer=bytearray(15)

    print "Image at ckpt1"
    print "###########################################"
    for t in range(20):
        if(not myFileSystem.blocksMetaData[t].free):
            myFileSystem.readBlock(t + 1, readBuffer)
            print t+1,readBuffer.decode('utf-8')
    print "###########################################"

    myFileSystem.createCheckpoint("ayushDisk","ckpt1")
    myFileSystem.writeDisk("ayushDisk",11,writeData5)
    myFileSystem.writeDisk("ayushDisk", 13, writeData6)
    myFileSystem.createCheckpoint("ayushDisk","ckpt2")
    myFileSystem.writeDisk("ayushDisk",15,writeData7)
    myFileSystem.writeDisk("ayushDisk", 17, writeData8)
    print "Image at ckpt3"
    print "###########################################"
    for t in range(20):
        if (not myFileSystem.blocksMetaData[t].free):
            myFileSystem.readBlock(t + 1, readBuffer)
            print t + 1, readBuffer.decode('utf-8')
    print "###########################################"
    myFileSystem.createCheckpoint("ayushDisk","ckpt3")


    myFileSystem.rollBack("ayushDisk","ckpt1")
    print "Image after rollback to ckpt1"
    print "###########################################"
    for t in range(20):
        if (not myFileSystem.blocksMetaData[t].free):
            myFileSystem.readBlock(t + 1, readBuffer)
            print t + 1, readBuffer.decode('utf-8')
    print "###########################################"
    myFileSystem.rollBack("ayushDisk", "ckpt3")
    print "Image after rollback to ckpt3"
    print "###########################################"
    for t in range(20):
        if (not myFileSystem.blocksMetaData[t].free):
            myFileSystem.readBlock(t + 1, readBuffer)
